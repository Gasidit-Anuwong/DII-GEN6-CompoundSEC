...
public class CardManagementGui extends JFrame {
    ...
    private final DefaultComboBoxModel<String> roomModel;
    ...
    private final JButton checkAccessButton;
    
    // Map สำหรับเก็บข้อมูลห้องที่ถูกจองแล้ว (Key: "floor_x", Value: Set ของ room numbers)
    ...
    // โหลดข้อมูลห้องที่ถูกจองแล้วจากโฟลเดอร์ "room"
    private void loadOccupiedRooms() {
        occupiedRooms.clear();
        File roomRoot = new File("room");
        if (roomRoot.exists() && roomRoot.isDirectory()) {
            File[] subfolders = roomRoot.listFiles(File::isDirectory);
            if (subfolders != null) {
                for (File subfolder : subfolders) {
                    // subfolder name คาดว่าจะเป็นในรูปแบบ "floor_x_y"
                    File statusFile = new File(subfolder, "status.txt");
                    if (statusFile.exists()) {
                        String[] parts = subfolder.getName().split("_");
                        if (parts.length >= 3) {
                            String floorKey = parts[0] + "_" + parts[1]; // เช่น "floor_1"
                            String roomNumber = parts[2];
                            Set<String> rooms = occupiedRooms.getOrDefault(floorKey, new HashSet<>());
                            rooms.add(roomNumber);
                            occupiedRooms.put(floorKey, rooms);
                        }
                    }
                }
            }
    ...
    // อัปเดตรายการห้องที่ว่างใน combo box
    ...
    // อัปเดตสถานะห้องใน GUI หลังจากมีการเปลี่ยนแปลง
    ...
    private class CardController {
        ...
        // ยกเลิกการจองห้อง (เฉพาะ Admin เท่านั้น)
